...
library(purrr)
y_pred = map(c(1:10),function(i){
                                      predict(fits[i],mnist_27$test)
                                    })

y_pred = as.data.frame(y_pred)
y_pred$y = mnist_27$test$y

y_accuracy = sapply(c(1:10), function(i){
  mean(y_pred$y==y_pred[,i])
})

names(y_accuracy) = colnames(y_pred)[1:10]
y_accuracy
mean(y_accuracy)

func_ensemble = function(r){
  return(ifelse(sum(r==7)>5,7,2))
}

y_pred$y_ensemble= apply(y_pred[,-11],1,func_ensemble)

y_ensemble_accuracy = mean(y_pred$y_ensemble == y_pred$y)
sum(y_accuracy>y_ensemble_accuracy)
y_accuracy[which(y_accuracy>y_ensemble_accuracy)]
...
dim(pca$x)
pca$x[,c(1:2)]
qplot(x=pca$x[,1],y=pca$x[,2], col=tissue_gene_expression$y)

qplot(x=pca$x[,1],y=rowMeans(tissue_gene_expression$x), col=tissue_gene_expression$y)
cor(pca$x[,1],rowMeans(tissue_gene_expression$x))

pca_result = as.data.frame(pca$x)
pca_result$y = tissue_gene_expression$y
View(pca_result)
pca_result %>% ggplot(aes(x=PC7, y = y)) + geom_boxplot()

summary(pca)


# MOVIE RECCO CASE STUDY
library(tidyverse)
library(dslabs)
data("movielens")
movielens %>% as_tibble()
...
random_selection = RMSE(test_set$rating, predictions)
random_selection

rmse_results <- tibble(method = c("Just the average","Any random rating - 3"), RMSE = c(naive_rmse, random_selection))
rmse_results
